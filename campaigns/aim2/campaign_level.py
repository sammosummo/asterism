...
# The direct path has moved into the cells: it is the thing under test
# here, and FIXED is merged last so anything named in both is decided
# by FIXED.
FIXED: dict[str, float] = {"d": 0.7, "sigma_m2": 0.5}
"""Fixed the outcome path and mediator residual variance across all cells."""
...
# Every cell here sets the direct path to nought, which is the horizontal
# null. What varies is whether the vertical estimand is also nought, because a
# test's level has to hold whatever the other estimand is doing -- a horizontal
# test that keeps its level only when there is no mediation is not much use in
# a study about mediation.
TRUTHS: dict[str, dict[str, float]] = {
    # Horizontal null, vertical null: the complete null, both ways round.
    "level: neither, no loading": {"a": 0.0, "b": 0.4, "c_prime": 0.0},
    "level: neither, no path": {"a": 0.6, "b": 0.0, "c_prime": 0.0},
    # Horizontal null while mediation is real and large. This is the one that
    # would expose a horizontal test borrowing evidence from the vertical path.
    "level: horizontal only": {"a": 0.6, "b": 0.4, "c_prime": 0.0},
}
"""Specified three level conditions spanning both estimands' null states."""
...
def main() -> int:
    """Run the selected level cells and write their count summary.
    ...
    """Enumerated every measurement-arm and level-condition combination."""
    ...
    """Initialised the count summary for each measurement-arm and level cell."""
    for arm in ARMS:
        for truth in TRUTHS:
    ...
    print(f"\nwritten to {OUT}")
    print(
        "\nEvery cell here has the direct path at nought, so rejection by the "
        "HORIZONTAL\ntest is its level and must sit at or below .025. The first two also\nhave the vertical estimand at nought, so vertical rejection there is a level\ntoo; in the third it is power."
    )
    return 0
