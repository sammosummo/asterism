...
# The direct path varies; the mediated effect stays real and large throughout.
# A direct-path test has to hold up while mediation is happening -- that is what
# separating them means -- so testing it only where there is nothing to separate
# would answer an easier question than the one the aim asks.
#
# Nought is included as the level cell, so every power figure here has a level
# measured under the identical design rather than borrowed from elsewhere.
TRUTHS: dict[str, dict[str, float]] = {
    "direct 0.0": {"a": 0.6, "b": 0.4, "c_prime": 0.0},
    "direct 0.3": {"a": 0.6, "b": 0.4, "c_prime": 0.3},
    "direct 0.5": {"a": 0.6, "b": 0.4, "c_prime": 0.5},
    "direct 0.7": {"a": 0.6, "b": 0.4, "c_prime": 0.7},
}
"""Specified the horizontal-path null and three direct-effect alternatives."""
...
def main() -> int:
    """Run the selected direct-path cells and write their count summary.
    ...
    """Enumerated every measurement-arm and direct-path combination."""
    ...
    """Initialised the count summary for each measurement-arm and path cell."""
    for arm in ARMS:
        for truth in TRUTHS:
