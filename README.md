...
| Capability | What you get | Runnable example |
| --- | --- | --- |
| **One trait, one component** | Heritability, its profile interval, and a test against zero | [`quickstart.py`](examples/quickstart.py) |
| **Several components** | Each component's share of variance, with intervals and tests — genes, a shared household, anything you can write as a matrix | [`components.py`](examples/components.py) |
| **Two traits** | Genetic, residual and phenotypic correlations, and a test that the genetic one is zero | [`bivariate.py`](examples/bivariate.py) |
...
| **Gene by environment** | How genetic effects change across an environment, measured or binary, and whether they change at all | [`gxe.py`](examples/gxe.py), [`discrete_gxe.py`](examples/discrete_gxe.py) |

Matrices can come from anywhere. `relationship_matrix` builds one from a
pedigree and `kinship_classes` splits a pedigree into separate bases, but every
model accepts any finite, symmetric, positive-semidefinite matrix — a genomic
relationship matrix, an estimated kinship, whatever you have. `align` lines a
matrix up with your values by identifier, which matters because the numerical
interface is positional and a misaligned fit does not warn, it silently
destroys the signal.

**[How to use each one is in models.md](docs/models.md).** Signatures are in the
generated [API reference](docs/api-reference.md), equations in
[statistical-methods.md](docs/statistical-methods.md), and the evidence behind
the numbers in [numerical-validation.md](docs/numerical-validation.md).
