...
def main() -> int:
    ...
    exited nought, which reads exactly like success -- and a whole branch's
    style debt was verified locally against no files at all. It is the fault
    this package keeps finding in its own models, a calculation that could not
    be made read as one that was, and it belongs to the gates too.
    """
